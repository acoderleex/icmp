...
jbyteArray
ping(JNIEnv *env, int sock, u_short sequence, const int size, jbyte *pattern, jsize pattern_len) {
...
//    return (int) ((time_end.tv_sec - time_start.tv_sec) * 1000.0 + elapsed);
    ...
    jbyteArray byteArray = env->NewByteArray(dataSize);
    env->SetByteArrayRegion(byteArray, 0, dataSize, (jbyte *) packet_data);
    return byteArray;
...
extern "C" jint
Java_com_tony_icmplib_Pinger_createicmpsocket(JNIEnv *env, jobject __unused thiz, jstring host,
...
extern "C" void
Java_com_tony_icmplib_Pinger_closeicmpsocket(JNIEnv *env, jobject __unused thiz, jint sock) {
    close_icmp_socket(sock);
}

extern "C" jbyteArray
Java_com_tony_icmplib_Pinger_ping(JNIEnv *env, jobject __unused thiz, jint sock, jchar sequence,
                                  jint size, jbyteArray pattern) {
    jbyte *const n_pattern = env->GetByteArrayElements(pattern, nullptr);
    const jsize pattern_len = env->GetArrayLength(pattern);

    jbyteArray result = ping(env, sock, sequence, size, n_pattern, pattern_len);
